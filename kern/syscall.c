...
//	panic("sys_exofork not implemented");
    ...
    int res = env_alloc(&newenv_store, curenv->env_id);
    if (res == 0) {
...
//	panic("sys_page_alloc not implemented");
    ...
    if ((perm & PTE_SYSCALL) == 0 || (perm & ~PTE_SYSCALL) != 0)
        return -E_INVAL;
    ...
    int res = page_insert(env_store->env_pgdir, pp, va, perm);
    if (res < 0) {
        page_free(pp);
        return -E_NO_MEM;
    }
...
//	panic("sys_page_map not implemented");
    ...
    struct Env *srcenv_store = NULL;
    int res = envid2env(srcenvid, &srcenv_store, 1);
    if (res < 0)
        return -E_BAD_ENV;

    struct Env *dstenv_store = NULL;
    res = envid2env(dstenvid, &dstenv_store, 1);
    if (res < 0)
        return -E_BAD_ENV;
    ...
    struct PageInfo *pp = page_lookup(srcenv_store->env_pgdir, srcva, &pte_store);
    if (!pp)
        return -E_INVAL;
    ...
    res = page_insert(dstenv_store->env_pgdir, pp, dstva, perm);
    if (res < 0)
        return -E_NO_MEM;
...
//	panic("sys_ipc_try_send not implemented");
    ...
    struct Env *dstenv_store = NULL;
    int res = envid2env(envid, &dstenv_store, 0);
    if (res < 0)
        return -E_BAD_ENV;

    if (!dstenv_store->env_ipc_recving || dstenv_store->env_ipc_from != 0)
    ...
    if ((uintptr_t) srcva < UTOP) {
        ...
        pte_t *pte_store = NULL;
        struct PageInfo *pp = page_lookup(curenv->env_pgdir, srcva, &pte_store);
        ...
        void *dstva = dstenv_store->env_ipc_dstva;
        res = page_insert(dstenv_store->env_pgdir, pp, dstva, perm);
        if (res < 0)
            return -E_NO_MEM;
    }

    dstenv_store->env_ipc_recving = 0;
